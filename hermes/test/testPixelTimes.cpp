...
void print_pixel_time(double t_pxl) {
	std::cerr << "Average pixel run time" << std::endl << "t_pxl = " << t_pxl << " ms" << std::endl;
}
...
TEST(PerformanceTest, DispersionMeasureIntegrator) {
	...
	double time = static_cast<double>(milliseconds.count()) / skymap->getNpix() * getThreadsNumber();
...
TEST(PerformanceTest, SynchroIntegrator) {
	int nside = 4;
	auto mfield = std::make_shared<magneticfields::JF12>(magneticfields::JF12());
...
TEST(PerformanceTest, PiZeroIntegrator) {
	...
	auto ringModel = std::make_shared<neutralgas::RingModel>(neutralgas::RingModel(neutralgas::GasType::H2));
	auto in = std::make_shared<PiZeroIntegrator>(PiZeroIntegrator(dragonModel, ringModel, kamae));
...
TEST(PerformanceTest, InverseComptonIntegrator) {
	...
	auto photonField = std::make_shared<photonfields::ISRF>(photonfields::ISRF(0));
	auto in =
	    std::make_shared<InverseComptonIntegrator>(InverseComptonIntegrator(dragonModel, photonField, kleinnishina));
	auto Egamma = 1_GeV;
...
TEST(PerformanceTest, BremsstrahlungIntegrator) {
	...
	std::vector<PID> particletypes = {Electron, Positron};
	auto dragonModel = std::make_shared<cosmicrays::Dragon2D>(cosmicrays::Dragon2D(particletypes));
	auto bremsstrahlung = std::make_shared<interactions::BremsstrahlungTsai74>(interactions::BremsstrahlungTsai74());
	auto ringModel = std::make_shared<neutralgas::RingModel>(neutralgas::RingModel(neutralgas::GasType::HI));
	// integrator
	auto in =
	    std::make_shared<BremsstrahlungIntegrator>(BremsstrahlungIntegrator(dragonModel, ringModel, bremsstrahlung));
	auto Egamma = 1_GeV;
	auto skymap = std::make_shared<GammaSkymap>(GammaSkymap(nside, Egamma));
	skymap->setIntegrator(in);
