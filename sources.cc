...
double TAstrophysicalSource::SourceDistribution(double x, double y, double z) {
  ...
  double deltaR = 0;
  if (Coord->GetType() == "2D") {
    deltaR = (Coord->GetR()[1])-(Coord->GetR()[0]);
  }
  double rad_sph = sqrt((radius*radius)+(z*z));
  double R1 = rad_sph - deltaR/2;
  double R2 = rad_sph + deltaR/2;
  double Rs = 20;
  double Rprim = sqrt((radius*radius)+(z*z/0.25));
  ...
  case NFWsquare:
    ...
    return pow(Rs,3)*(1/pow(1+R1/Rs,3) - 1/pow(1+R2/Rs,3))/(pow(R2,3)-pow(R1,3));
    break;
  
  case ISOsq:
    return pow(1/(1+pow(rad_sph/3.5,2)),2);
    break;
    
  case McMillan:
    return ( (98.4*exp(-((radius*radius)+(z*z/0.25))/4.41)/pow(1+ (Rprim/0.075),1.8)) + (896/(2.0*300))*exp(-(fabs(z)/0.3)-(radius/2.5)) + (183/(2.0*900))*exp(-(fabs(z)/0.9)-(radius/3.02)) );
    break;
