        ...
        for (int i = 0; i < dimx; i++) {
            for (int k = 0; k < dimz; k++) {
                ...
                if (x[i]<5){
                    ...
                    bTurbDisk = 7.63e-6 * 5/x[i];
                }
                bTurbDisk *= exp(-pow(z[k]/0.61,2)/2);
                double bTurbHalo = 4.68e-6* exp(-x[i]/10.97)*exp(-pow(z[k]/2.84,2)/2);
                double Bav2 = pow(Bav[0],2)+pow(Bav[1],2)+pow(Bav[2],2)+pow(bTurbDisk,2)+pow(bTurbHalo,2);
