...
int Input::LoadFile(const string inputfilename) {
	...
	if (doc->LoadFile(inputfilename.c_str(), TIXML_DEFAULT_ENCODING)) {
		...
		OnlyPrimaries = false;
		OnlyPrimaries = el->FirstChildElement("OnlyPrimaries");
		...
		else if (gmod == "Hermes") gas_model = Hermes;
		...
		else if (gmod == "NFWsquare") SNR_model = NFWsquare;
		else if (gmod == "ISOsq") SNR_model = ISOsq;
		else if (gmod == "McMillan") SNR_model = McMillan;
		...
		if (el1)
		{
			...
			else if (gmod == "NFWsquare") SNR_model_Extra = NFWsquare;
			else if (gmod == "McMillan") SNR_model_Extra = McMillan;
			else if (gmod == "ISOsq") SNR_model_Extra = ISOsq;
